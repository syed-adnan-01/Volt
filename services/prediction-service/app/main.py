...
# Lyzr AI explainer agent - wrapped safely so it can never break /predict
try:
    lyzr_studio = Studio(api_key=os.getenv("LYZR_API_KEY"))
    explainer_agent = lyzr_studio.create_agent(
...
def predict_one(request: PredictionRequest):
    ...
    # Lyzr AI explanation - safely wrapped so a Lyzr issue can NEVER break /predict
    response["explanation"] = None
    if explainer_agent is not None:
        try:
            explanation_prompt = (
                f"Station {response['stationId']}: "
                f"{response['availabilityProbability']*100:.0f}% chance available, "
                f"expected wait {response['expectedWaitMinutes']} minutes, "
                f"reliability {response['reliabilityScore']*100:.0f}%, "
                f"confidence {response['confidence']*100:.0f}%."
            )
            explanation_result = explainer_agent.run(explanation_prompt)
            response["explanation"] = explanation_result.response
        except Exception:
            response["explanation"] = None
